...
export default class QuizController {
    ...
    @Post('/register')
    CreateUser(@Body() createUserData: createUserDto, @Session() session: Record<string, any>) {
        return this.quizService.createUser(createUserData, session)
    }

    @Post('/login')
    LoginUser(@Body() userData: loginUserDto, @Session() session: Record<string, any>) {
        return this.quizService.loginUser(userData, session)
    }


    @Get('/user')
    User(@Session() session: Record<string, any>) {
        if (!session.user) throw new UnauthorizedException()
        return session.user;
    }

    @Get('/question')
    QuestionHandler(@Session() session: Record<string, any>) {
        if (!session.user) throw new UnauthorizedException()
        return this.quizService.getRandomQuestionForUser(session.user._id)
    }

    @Post('/submit_answer')
    SubmitAnswer(@Body() SubmitAnswerDto: SubmitAnswerDto, @Session() session: Record<string, any>) {
        if (!session.user) throw new UnauthorizedException()
        return this.quizService.submitAnswer(SubmitAnswerDto, session.user._id)
    }

    @Get('/start_play')
    StartPlay(@Session() session: Record<string, any>) {
        if (!session.user) throw new UnauthorizedException()
        return this.quizService.startGame(session.user._id)
    }

    @Get('/finished')
    GameFinshed(@Session() session: Record<string, any>) {
        if (!session.user) throw new UnauthorizedException()
